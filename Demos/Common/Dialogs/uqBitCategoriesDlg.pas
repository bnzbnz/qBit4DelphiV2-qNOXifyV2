...
type
  ...
  public
    ...
    function ShowAsModal(qB: TqBit; SelectMode: Boolean = True): TModalResult;
  end;
...
begin
  ...
  Selected := TqBitCategoryType(FCatList.categories.Items[ LBCat.Items[LBCat.ItemIndex] ]).Clone<TqBitCategoryType>;
  ModalResult := mrOK;
end;

procedure TqBitCategoriesDlg.Button1Click(Sender: TObject);
var
  Temp: TqBitCategoryType;
begin
  if Trim(EditName.Text) = '' then Exit;
  if FCatList.categories.TryGetValue(EditName.Text, Temp) then
  begin
...
begin
  ...
  var ActiveCat := TqBitCategoryType(FCatList.categories.Items[ LBCat.Items[LBCat.ItemIndex] ]);
  EditName.Text := ActiveCat.name.AsString;
  EditPath.Text := ActiveCat.savePath.AsString;
end;
...
begin
  ...
  LBCat.Clear;
  LBCat.Sorted := True;
  for var Cat in FCatList.categories do
...
function TqBitCategoriesDlg.ShowAsModal(qB: TqBit; SelectMode: Boolean): TModalResult;
begin
  qBit := qB;
